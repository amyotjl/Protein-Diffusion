...
class Autoencoder(pl.LightningModule):
    ...
    def __init__(
        ...
        for _ in range(len(in_channels)-1):
            out_dim.append(self.compute_dim(out_dim[-1], self.conv_k, self.conv_s, self.conv_p))

        in_channels_reversed = in_channels[::-1]
        out_dim_reversed = out_dim[::-1]
        self.encoder = encoder_class(out_dim, in_channels, latent_dim, conv_k, conv_s, conv_p, use_batch_norm, dropout_rate, activation_function)
        self.decoder = decoder_class(out_dim_reversed, in_channels_reversed, latent_dim, conv_k, conv_s, conv_p, use_batch_norm, dropout_rate, activation_function)
    ...
    def compute_dim(self, dim, conv_k, conv_s, conv_p):
